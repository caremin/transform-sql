CREATE OR REPLACE VIEW public.part_attend_batch
AS SELECT p.id AS sys_participant_id,
    concat(c.community_id, lpad(p.participant_id::text, 2, '0'::text)) AS participant_id,
  ...
  WHERE b.id = 28 AND bases.name::text <> 'Manila - Beta Test'::text AND p.status::text <> 'disqualified'::text AND p.deleted_at IS NULL;