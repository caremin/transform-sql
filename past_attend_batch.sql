CREATE OR REPLACE VIEW public.past_attend_batch
AS SELECT p.id AS sys_participant_id,
    concat('past', p.id::text) AS fixed_part_past_id,
    c.community_id AS participant_id,
    NULL::unknown AS original_participant_id,
    ...
    "left"(now()::character varying::text, 4)::integer - "left"(p.birth_date::text, 4)::integer AS participant_age_est,
    p.birth_date AS part_birthdate,
    NULL::unknown AS part_phonenumber,
    'Pastor' AS part_type,
    'unknown' AS part_status,
    NULL::unknown AS part_isvisitor,
    NULL::unknown AS part_income,
    NULL::unknown AS part_povscore,
    NULL::unknown AS part_selectionscore,
    'pastor' AS part_sg_role,
   ...
   FROM pastor_attendances pa
     LEFT JOIN pastors p ON p.id = pa.pastor_id
     LEFT JOIN communities c ON pa.community_id = c.id
